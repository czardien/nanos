    ...
    if (params && (*(u16 *)(params + BOOT_PARAM_OFFSET_BOOT_FLAG) == 0xAA55) &&
        ...
        for (u8 entry = 0; entry < e820_entries; entry++) {
            ...
            if ((r->type = REGION_PHYSICAL) && (r->base <= KERNEL_BASE_PHYS) &&
                ...
                if (KERNEL_BASE_PHYS - r->base >= 2 * PAGESIZE) {
                    /* There is a gap between start of memory region and start
                     * of kernel code: use part of it as storage for a set of
                     * temporary page tables that we need to set up an initial
                     * mapping of the kernel virtual address space, and make the
                     * remainder a new memory region. */
